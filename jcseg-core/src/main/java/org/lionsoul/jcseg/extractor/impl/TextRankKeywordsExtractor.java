...
public class TextRankKeywordsExtractor extends KeywordsExtractor
{
    ...
    */
     ...
     * @throws    IOException
     */
    public List<String> getKeywordsFromString(String doc, Integer weight) throws IOException
    {
        return getKeywords(new StringReader(doc), weight);
    }

    public List<String> getKeywords(Reader reader, Integer weight) throws IOException {

        Map<String, List<String>> winMap = new HashMap<String, List<String>>();
        List<String> words = new ArrayList<String>();
        HashMap<String, Float> score = textRankCalc(reader, winMap, words);
        ...
        for ( int i = 0; i < entryList.size(); i++ )
        {
            ...
            keywords.add(e.getKey() + "_" + (e.getValue() * weight));
        }
    ...
    public List<String> getKeywords(Reader reader) throws IOException 
    {
        ...
        for ( int i = 0; i < entryList.size(); i++ )
        {
            ...
            if ( i >= len ) break;
            if ( autoFilter && e.getValue() < stdScores ) break;
            keywords.add(e.getKey() + "_" + e.getValue());
        }
    ...
    private HashMap<String, Float> textRankCalc(Reader reader, Map<String, List<String>> winMap, List<String> words) throws IOException {
        //document segment
        IWord w = null;
        seg.reset(reader);
